    ...
    function fetchUserMap() {
        return db.collection('users').get().then(snapshot => {
            ...
            snapshot.forEach(doc => {
                ...
                // fetchUserMap içindeki ilgili alanı tam olarak bu satırla değiştirin:
userMap[doc.id] = { isim: player.isim || player.email, email: player.email, uid: doc.id, toplamPuan: player.toplamPuan, ciftlerPuani: player.ciftlerPuani, kortTercihi: player.kortTercihi, telefon: player.telefon, fotoURL: player.fotoURL, cinsiyet: player.cinsiyet, bildirimTercihi: player.bildirimTercihi || 'ses', tenisBaslangic: player.tenisBaslangic || '', kulup: player.kulup || 'Belirtilmemiş', emailNotifications: (player.emailNotifications !== false), macSayisi: player.macSayisi || 0, galibiyetSayisi: player.galibiyetSayisi || 0, rol: player.rol || 'oyuncu', badges: player.badges || [] }; 
...
adminArea.innerHTML = `
            ...
            ${tourData.prizeDistributed ? 
                `<p style="background:#e8f5e9; color:#28a745; padding:10px; border-radius:8px; font-weight:bold; text-align:center; margin:10px 0; font-size:0.85em;">💰 Ödül Havuzu Başarıyla Dağıtıldı</p>` : 
                `<button onclick="window.manualDistributePrizePool('${tourId}')" class="btn-main" style="background: linear-gradient(135deg, #FF9800 0%, #F57C00 100%); color:white; padding:10px; font-size:0.85em; margin-bottom:10px; box-shadow: 0 4px 10px rgba(245,124,0,0.3); width:100%;">🏆 Turnuva Ödül Havuzunu Dağıt</button>`
            }
...
async function joinTournament(tourId, tourData, myUid) {
    ...
    const isDoublesTour = !(tourData.format || '').includes('Tekler');
    const pointField = isDoublesTour ? 'ciftlerPuani' : 'toplamPuan';
    const myPoints = isDoublesTour ? (me.ciftlerPuani !== undefined ? me.ciftlerPuani : 1000) : (me.toplamPuan || 0);

    if (myPoints < tourData.fee) return alert(`Bu turnuvaya katılmak için en az ${tourData.fee} turnuva puanına ihtiyacın var.`);
    ...
    if (!format.includes('Tekler') && finalRegType === 'auto') {
        ...
        if (!p2) return alert("Partner bulunamadı.");
        
        const p2Points = isDoublesTour ? (p2.ciftlerPuani !== undefined ? p2.ciftlerPuani : 1000) : (p2.toplamPuan || 0);
        if (p2Points < tourData.fee) return alert(`Partnerinizin puanı bu turnuva ücreti (${tourData.fee} P) için yetersiz!`);

        if (format === 'Double Erkek' && (myGender !== 'Erkek' || p2.cinsiyet !== 'Erkek')) return alert("Hata: Sadece Erkek-Erkek.");
        if (format === 'Double Kadın' && (myGender !== 'Kadın' || p2.cinsiyet !== 'Kadın')) return alert("Hata: Sadece Kadın-Kadın.");
    ...
    try {
        ...
        await batch.commit();
        alert(`Turnuvaya başarıyla kaydoldunuz! Giriş ücreti (${tourData.fee} P) hesabınızdan tahsil edildi. 🏆`);
        ...
        if(document.getElementById('btn-join-tour')) document.getElementById('btn-join-tour').disabled = false;
    }
}

window.cancelTournamentRegistration = async function(tourId, p1, p2) {
    if (!confirm("Turnuva kaydını iptal etmek istediğine emin misin? Ücretiniz iade edilecektir.")) return;
    try {
        const docRef = db.collection('tournaments').doc(tourId); const doc = await docRef.get();
        const tourData = doc.data();
        let parts = tourData.participants || [];
        ...
        const batch = db.batch();
        batch.update(docRef, { participants: parts });
        if(p1) batch.update(db.collection('users').doc(p1), { [pointField]: firebase.firestore.FieldValue.increment(tourData.fee) });
        if(p2 && p2 !== 'null') batch.update(db.collection('users').doc(p2), { [pointField]: firebase.firestore.FieldValue.increment(tourData.fee) });
        
        await batch.commit();
        alert("Kaydınız iptal edildi ve giriş ücretiniz iade edildi. ↩️");
        openTournamentDetail(tourId, (await docRef.get()).data());
    } catch(e) { alert("İptal sırasında hata oluştu: " + e.message); }
};

window.adminDeleteParticipant = async function(tourId, index) {
    if (!confirm("Bu kaydı turnuvadan çıkarmak istediğinize emin misiniz? Ücreti iade edilecektir.")) return;
    try {
        const docRef = db.collection('tournaments').doc(tourId); const doc = await docRef.get();
        const tourData = doc.data();
        let parts = tourData.participants || []; 
        const removedTarget = parts[index];
        ...
        const batch = db.batch();
        batch.update(docRef, { participants: parts });
        if(removedTarget.p1) batch.update(db.collection('users').doc(removedTarget.p1), { [pointField]: firebase.firestore.FieldValue.increment(tourData.fee) });
        if(removedTarget.p2) batch.update(db.collection('users').doc(removedTarget.p2), { [pointField]: firebase.firestore.FieldValue.increment(tourData.fee) });

        await batch.commit(); 
        alert("Kayıt silindi ve puanı iade edildi. 🗑️");
        openTournamentDetail(tourId, (await docRef.get()).data());
...
window.adminAddParticipant = async function(tourId) {
    ...
    // Çift tıklamaları önlemek için butonu kilitliyoruz
    const btnSave = document.getElementById('btn-admin-save-reg');
    if(btnSave) { btnSave.disabled = true; btnSave.textContent = "Ekleniyor..."; }
    ...
    try {
        ...
        // --- GÜVENLİK KİLİDİ 1: OYUNCU ZATEN KAYITLI MI? ---
        const participants = tourData.participants || [];
        const p1AlreadyIn = participants.some(pt => pt.p1 === p1 || pt.p2 === p1);
        if (p1AlreadyIn) throw new Error("Oyuncu 1 zaten bu turnuvaya kayıtlı!");
        
        if (p2) {
            const p2AlreadyIn = participants.some(pt => pt.p1 === p2 || pt.p2 === p2);
            if (p2AlreadyIn) throw new Error("Oyuncu 2 (Partner) zaten bu turnuvaya kayıtlı!");
        }

        const p1Gender = userMap[p1]?.cinsiyet;
        if (!p1Gender) throw new Error("Seçilen oyuncunun cinsiyet bilgisi eksik.");

        if (format === 'Tekler Erkek' && p1Gender !== 'Erkek') throw new Error("Sadece Erkek oyuncu seçebilirsiniz.");
        if (format === 'Tekler Kadın' && p1Gender !== 'Kadın') throw new Error("Sadece Kadın oyuncu seçebilirsiniz.");

        let finalRegType = regType;
        if (regType === 'hybrid') { finalRegType = p2 ? 'manual' : 'auto'; }

        if (!format.includes('Tekler') && finalRegType === 'auto') {
            if (format === 'Double Erkek' && p1Gender !== 'Erkek') throw new Error("Sadece erkek oyuncu seçebilirsiniz.");
            if (format === 'Double Kadın' && p1Gender !== 'Kadın') throw new Error("Sadece kadın oyuncu seçebilirsiniz.");
        } 
        else if (!format.includes('Tekler') && finalRegType === 'manual') {
            if (!p2) throw new Error("Bu formatta partner seçmek zorunludur.");
            const p2Gender = userMap[p2]?.cinsiyet;
            if (!p2Gender) throw new Error("Partnerin cinsiyet bilgisi eksik.");

            if (format === 'Double Erkek' && (p1Gender !== 'Erkek' || p2Gender !== 'Erkek')) throw new Error("Sadece Erkek-Erkek.");
            if (format === 'Double Kadın' && (p1Gender !== 'Kadın' || p2Gender !== 'Kadın')) throw new Error("Sadece Kadın-Kadın.");
            if (format === 'Mix' && (p1Gender === p2Gender)) throw new Error("Mix için 1 Kadın 1 Erkek gerekli.");
        }

        // --- GÜVENLİK KİLİDİ 2: PUAN YETERLİ Mİ? ---
        const p1Points = isDoublesTour ? (userMap[p1].ciftlerPuani !== undefined ? userMap[p1].ciftlerPuani : 1000) : (userMap[p1].toplamPuan || 0);
        if (p1Points < tourData.fee) throw new Error(`Oyuncu 1'in puanı (${p1Points} P) yetersiz!`);

        if (isDoublesTour && finalRegType === 'manual' && p2) {
            const p2Points = userMap[p2].ciftlerPuani !== undefined ? userMap[p2].ciftlerPuani : 1000;
            if (p2Points < tourData.fee) throw new Error(`Seçilen partnerin puanı (${p2Points} P) yetersiz!`);
        }
        ...
        alert("İşlem iptal edildi: " + e.message); 
    } finally {
        // İşlem bitince butonu tekrar aktifleştir
        if(btnSave) { btnSave.disabled = false; btnSave.textContent = "Kaydı Ekle ✅"; }
    }
};
...
window.advanceTournamentBracket = async function(tourId, matchTag, winnerUid, isSync = false) {
    ...
    // 1. ADIM: Maçın gerçek Firestore ID'sini turnuva ağacından buluyoruz (ID Çakışma Çözümü)
    let firestoreMatchId = null;
    if (matchTag.startsWith('G')) {
        const gIdx = parseInt(matchTag.split('_')[0].replace('G',''));
        const mObj = data.groups?.[gIdx]?.matches?.find(m => m.matchId === matchTag);
        if (mObj) firestoreMatchId = mObj.firestoreMatchId;
    } else if (matchTag.startsWith('L') || matchTag.startsWith('R')) {
        const rIdx = parseInt(matchTag.split('_')[0].replace(/[LR]/,''));
        const mIdx = parseInt(matchTag.split('_')[1].replace('M',''));
        firestoreMatchId = data.bracket?.[rIdx]?.matches?.[mIdx]?.firestoreMatchId;
    }

    // 2. ADIM: Doğrudan doğru maç dökümanını çekiyoruz
    let mData = null;
    if (firestoreMatchId) {
        const mDoc = await db.collection('matches').doc(firestoreMatchId).get();
        if (mDoc.exists) mData = mDoc.data();
    }

    // A. GRUP MAÇI İŞLEYİCİSİ
    if (matchTag.startsWith('G')) {
...
// İstatistik Havuzunu Kur
    ...
    html += `
        <div class="bracket-match" style="cursor:pointer; border:1px solid ${match.winner ? '#28a745' : '#c06035'}; min-width:unset; width:100%;" onclick="returnToTab = 'tab-tournaments'; window.verifyAndShowMatch('${tourId}', '${match.matchId || match.matchTag}', '${match.firestoreMatchId}')">
...
window.recalculateAllPoints = async function() {
    ...
    try {
        ...
        loading.innerHTML = '<div>Lig verileri taranıyor ve puanlar sıfırdan inşa ediliyor...</div><div style="font-size:0.8em; color:#ff9800; margin-top:10px;">Lütfen sekmeyi kapatmayın ⏳</div>';
        document.body.appendChild(loading);

        // 1. ADIM: Tüm kullanıcıları başlangıç puanına (1000) sıfırla
        const usersSnap = await db.collection('users').get();
        let userStats = {};
        usersSnap.forEach(doc => {
            userStats[doc.id] = { toplamPuan: 1000, ciftlerPuani: 1000, galibiyetSayisi: 0, macSayisi: 0, ref: doc.ref };
        });

        // 2. ADIM: Tüm tamamlanmış maçları işle
        const matchesSnap = await db.collection('matches').where('durum', '==', 'Tamamlandı').get();
        matchesSnap.forEach(doc => {
            const m = doc.data();
            const wid = m.kayitliKazananID;
            ...
            const bonusW = (isNaN(wg) ? 0 : wg) * 5; 
            const bonusL = (isNaN(lg) ? 0 : lg) * 5;
            let winPoints = 50 + bonusW; 
            let losePoints = 50 + bonusL;
            if(m.macTipi === 'Meydan Okuma') { winPoints = (m.bahisPuani||0) + bonusW; losePoints = -(m.bahisPuani||0) + bonusL; }
            ...
            applyToUser(wid, true); applyToUser(lid, false);
            if (!(m.macFormati || '').includes('Tekler')) { applyToUser(wPartnerId, true); applyToUser(lPartnerId, false); }
        });

        // 3. ADIM [EKSİK OLAN KRİTİK ADIM]: Turnuva Katılım Ücretlerini Düş
        const allToursSnap = await db.collection('tournaments').get();
        allToursSnap.forEach(tourDoc => {
            ...
            (tourData.participants || []).forEach(p => {
                if (p.p1 && userStats[p.p1]) userStats[p.p1][pointField] -= fee;
                if (p.p2 && userStats[p.p2]) userStats[p.p2][pointField] -= fee;
            });
        });

        // 4. ADIM: Ödülü Dağıtılmış Turnuvaları Bul ve Büyük Havuz Puanlarını Geri Ekle
        const prizeToursSnap = await db.collection('tournaments').where('prizeDistributed', '==', true).get();
        prizeToursSnap.forEach(tourDoc => {
            const tourData = tourDoc.data();
            const fee = tourData.fee || 0;
            const participants = tourData.participants || [];
            ...
            participants.forEach(p => { if (p.p1) totalPlayersCount++; if (p.p2) totalPlayersCount++; });
            ...
            if (tourData.systemType === 'league' && tourData.bracket) {
                ...
                if (finalMatch && finalMatch.winner) {
                    ...
                    addPrizeToMemory(championTeam, Math.round(totalPrizePool * 0.60));
                    const finalistTeam = (finalMatch.p1.p1 === championTeam.p1) ? finalMatch.p2 : finalMatch.p1;
                    addPrizeToMemory(finalistTeam, Math.round(totalPrizePool * 0.25));
                    if (tourData.bracket.length >= 2) {
                        ...
                        semiRound.matches.forEach(m => {
                            ...
                            if(loserTeam && !loserTeam.isBye) addPrizeToMemory(loserTeam, Math.round(totalPrizePool * 0.075));
                        });
                    }
                }
            }
        ...
        // 5. ADIM: Hesaplanan Kusursuz Yeni Puanları Veritabanına Topluca Yazdır
        const batch = db.batch();
        ...
        alert("Muazzam! Tüm lig puanları, katılım ücretleri ve turnuva ödül havuzları taranarak sıfırdan başarıyla onarıldı! 🚀👑");
...
// ============================================================================
  ...
  // --- 4. GRUPLAR İLK OLUŞTUĞUNDA TÜM GRUP LİSTELERİNİ WHATSAPP'A ATAN MOTOR ---
    ...
    // --- 6. LİG FİKSTÜRÜ İLK OLUŞTUĞUNDA 1. HAFTA MAÇLARINI ATAN MOTOR ---
    window.sendWhatsAppLeagueDrawNotification = async function(tournamentName, bracket) {
        const WA_API_URL = "https://7107.api.greenapi.com"; 
        const WA_INSTANCE_ID = "7107628348";                
        const WA_API_TOKEN = "fee80956785a47639c4bd62e63886be7c5c2ef330fc64dce9c"; 
        const WA_RECIPIENT_CHAT_ID = "120363425128455544@g.us"; // Canlı grup ID'niz
        ...
        // Fikstürün sadece 1. Haftasındaki (bracket[0]) tüm maçları çekip listeliyoruz
        if (bracket && bracket[0] && bracket[0].matches) {
            bracket[0].matches.forEach((m, index) => {
                const getPlayerFullName = (p) => {
                    if (!p) return "Bekleniyor";
                    if (p.isBye) return "- BAY (Maçsız Geçer) -";
                    const p1 = userMap[p.p1]?.isim || 'Bilinmeyen';
                    const p2 = p.p2 ? ` & ${userMap[p.p2]?.isim || 'Bilinmeyen'}` : '';
                    return `${p1}${p2}`;
                };
        ...
        const messageText = 
            `📅 *LİG SEZONU RESMEN BAŞLADI!* 📅\n\n` +
            `🏆 *${tournamentName}* lig usulü turnuvamızda dev kura çekildi ve haftalık fikstür tamamen oluşturuldu!\n\n` +
            `🔥 *1. Hafta Açılış Karşılaşmaları:* \n\n` +
            matchesText +
            `👉 _Tüm haftaların fikstürünü, anlık puan durumunu ve kort detaylarını görmek için hemen uygulamaya giriş yapabilirsiniz. Tüm oyuncularımıza başarılar dileriz!_ 🎾`;
        ...
        try {
            ...
            if (response.ok) console.log("Lig fikstür kuraları başarıyla WhatsApp grubuna fırlatıldı! 🚀");
        } catch (error) { console.error("💥 Lig kura bildirimi atılırken ağ hatası:", error); }
    };

// ============================================================================
    // ========== 7. CANLI MAÇ SONUCU + GÜNCEL KAZANMA YÜZDELİ TABLO MOTORU =======
    // ============================================================================
    window.sendWhatsAppMatchResultAndStandingsNotification = async function(tourId, matchTag, completedMatch) {
        ...
        try {
            ...
            // SENARYO C: GRUP AŞAMASINDA OLAN TURNUVA (Çizgiler Kalktı, white_circle -> ⚪ Düzeldi)
                ...
                sortedPlayers.forEach((p, pIdx) => {
                    const advClass = pIdx < (tourData.advancingCount || 2) ? "🟢" : "⚪"; // white_circle hatası ⚪ emojisi ile düzeltildi
                    const scoreLabel = isThreePoint 
                        ? `Puan: *${p.groupPoints || 0} P* •  Kazanma Oranı: *%${(p.winRate || 0).toFixed(1)}*` 
                        : `Kazanma Oranı: *%${(p.winRate || 0).toFixed(1)}*`;
                    
                    messageText += `${advClass} *${pIdx + 1}. ${getPlayerFullNameLocal(p)}*\n`;
                    messageText += `📝 Maç: *${p.played}* •  G: *${p.won}* •  M: *${p.lost}* •  Av: *${p.gamesWon}-${p.gamesLost}*\n`;
                    messageText += `🎯 ${scoreLabel}\n\n`;
                });

                messageText += `🌳 *ELEME AĞACI (ADAY HARİTASI)*\n`;
                tourData.bracket.forEach(round => {
                    messageText += `\n📌 *${round.roundName.toUpperCase()}*\n`;
                    round.matches.forEach((m, idx) => {
                        const winnerText = m.winner ? `👉 🎉 *[${getPlayerFullNameLocal(m.winner)}]*` : `👉 ⏳ _Bekliyor_`;
            ...
            if (response.ok) console.log("Yenilenmiş şık tasarımlı canlı rapor WhatsApp grubuna başarıyla fırlatıldı! 🚀");
...
// --- 16. PROFESYONEL ÖDÜL HAVUZU DAĞITIM MOTORU (MANUEL VE OTOMATİK UYUMLU) ---
window.distributeTournamentPrizePool = async function(tourId, tourData) {
    if (!tourId || !tourData) return;
    if (tourData.prizeDistributed === true) {
        console.log("Bu turnuvanın ödül havuzu zaten dağıtılmış. ⏩");
        return;
    }
    ...
    // Toplam katılan tekil oyuncu sayısını hesapla
    let totalPlayersCount = 0;
    participants.forEach(p => {
    ...
    const batch = db.batch();
    let winnersSummaryText = `💰 *TOPLAM ÖDÜL HAVUZU:* ${totalPrizePool} Puan\n\n`;

    // Ödül ekleme yardımcı fonksiyonu (Çiftlerse ödülü ikiye böler)
    const awardPrize = (teamObj, prizeAmount, label) => {
        if (!teamObj || !teamObj.p1) return;
        let perPlayerPrize = prizeAmount;
        if (isDoubles && teamObj.p2) perPlayerPrize = Math.round(prizeAmount / 2);

        batch.update(db.collection('users').doc(teamObj.p1), { [pointField]: firebase.firestore.FieldValue.increment(perPlayerPrize) });
        let names = userMap[teamObj.p1]?.isim || 'Bilinmeyen';
        
        if (isDoubles && teamObj.p2) {
            batch.update(db.collection('users').doc(teamObj.p2), { [pointField]: firebase.firestore.FieldValue.increment(perPlayerPrize) });
            names += ` & ${userMap[teamObj.p2]?.isim || 'Bilinmeyen'}`;
        }
        winnersSummaryText += `🏅 *${label}:* ${names} (+${prizeAmount} P)\n`;
    };

    // --- SENARYO A: LİG USULÜ TURNUVA (Podyum Modeli: %50, %30, %20) ---
    if (tourData.systemType === 'league') {
        // Lig puan durumu istatistiklerini simüle et ve sırala
        let stats = {};
        tourData.bracket.forEach(round => {
            round.matches.forEach(m => {
                if(!m.p1 || !m.p2) return;
                const p1Id = m.p1.p1 + (m.p1.p2 ? "_" + m.p1.p2 : "");
                const p2Id = m.p2.p1 + (m.p2.p2 ? "_" + m.p2.p2 : "");
                if(!stats[p1Id]) stats[p1Id] = { obj: m.p1, pts: 0 };
                if(!stats[p2Id]) stats[p2Id] = { obj: m.p2, pts: 0 };
                if (m.winner) {
                    if (m.winner.p1 === m.p1.p1) { stats[p1Id].pts += 3; stats[p2Id].pts += 1; }
                    else { stats[p2Id].pts += 3; stats[p1Id].pts += 1; }
                }
            });
        });
        const sorted = Object.values(stats).sort((a,b) => b.pts - a.pts);
        
        if(sorted[0]) awardPrize(sorted[0].obj, Math.round(totalPrizePool * 0.50), "Lig Şampiyonu 🥇");
        if(sorted[1]) awardPrize(sorted[1].obj, Math.round(totalPrizePool * 0.30), "Lig İkincisi 🥈");
        if(sorted[2]) awardPrize(sorted[2].obj, Math.round(totalPrizePool * 0.20), "Lig Üçüncüsü 🥉");
    } 
    // --- SENARYO B: ELEMELİ / GRUPLU TURNUVA (ATP Kademeli Model: %60, %25, %7.5, %7.5) ---
    else if (tourData.bracket && tourData.bracket.length > 0) {
        const finalRound = tourData.bracket[tourData.bracket.length - 1];
        const finalMatch = finalRound?.matches?.[0];
        if (!finalMatch || !finalMatch.winner) {
            throw new Error("Turnuva ağacı henüz tamamlanmadığı için ödül dağıtılamaz.");
        }

        // 1. Şampiyon (%60)
        const championTeam = finalMatch.winner;
        awardPrize(championTeam, Math.round(totalPrizePool * 0.60), "Turnuva Şampiyonu 👑");

        // 2. Finalist (%25)
        const finalistTeam = (finalMatch.p1.p1 === championTeam.p1) ? finalMatch.p2 : finalMatch.p1;
        awardPrize(finalistTeam, Math.round(totalPrizePool * 0.25), "Finalist / İkinci 🥈");

        // 3. Yarı Finalistler (Son turdan bir önceki turun elenenleri - Her birine %7.5)
        if (tourData.bracket.length >= 2) {
            const semiRound = tourData.bracket[tourData.bracket.length - 2];
            semiRound.matches.forEach(m => {
                const loserTeam = (m.winner?.p1 === m.p1?.p1) ? m.p2 : m.p1;
                if(loserTeam && !loserTeam.isBye) {
                    awardPrize(loserTeam, Math.round(totalPrizePool * 0.075), "Yarı Finalist 🥉");
                }
            });
        }
    }

    // Çifte dağıtımı engellemek için kilidi vur
    batch.update(db.collection('tournaments').doc(tourId), { prizeDistributed: true });
...
window.manualDistributePrizePool = async function(tourId) {
    if(!confirm("Mevcut açık/biten turnuvanın ödül havuzunu şu anki sıralamaya göre dağıtmak istediğinize emin misiniz?")) return;
    try {
        const snap = await db.collection('tournaments').doc(tourId).get();
        if(!snap.exists) return alert("Turnuva bulunamadı.");
        const tourData = snap.data();
        
        await window.distributeTournamentPrizePool(tourId, tourData);
        alert("Ödül havuzu milimetrik olarak hesaplandı, kazananlara dağıtıldı ve WhatsApp grubuna tebrik raporu uçuruldu! 🚀🏆");
